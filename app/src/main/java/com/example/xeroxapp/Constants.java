package com.example.xeroxapp;

 public class Constants {
    //private static final String ROOT_URL = " http://192.168.0.107/xeroxapp/";
    private static final String ROOT_URL = "http://www.pccoerelearning.com/student/copygram/";
   //private static final String ROOT_URL = " https://copygram.000webhostapp.com/copygram/";
    public static final String REGISTER_URL = ROOT_URL+"register_user.php";

     public static final String LOGIN_URL = ROOT_URL+"login_user.php";

     public static final String UPLOAD_URL = ROOT_URL+"save_file.php";

     public static final String FETCH_URL = ROOT_URL+"fetchfile.php";

     public static final String UPDATE_FILE_DETAILS_URL = ROOT_URL+"update_fileinfo.php";

     //public static final String CHECKSUM_URL = ROOT_URL+"paytm/generateChecksum.php";

     public static final String PAYMENT_URL = ROOT_URL+"paymentDetails.php";

     public static final String FETCH_ORDER_URL = ROOT_URL+"fetchOrder.php";

     public  static final String DELETE_FILE_URL = ROOT_URL+"deleteFile.php";

     public static final String MERCHANT_UPI = "vibrus@sbi";

     public static String Email;

}